...
def search_post_data(terms):
    ...
    title_union = [title_query.format(term.replace('& ', '').replace('*', ''), term) for term in terms]
    para_union = [para_query.format(term.replace('& ', '').replace('*', ''), term) for term in terms]
...
def parse_results_for_highlights(result_data):
    """
    Put <span> tags around searched text in resulting title or paragraph for visual highlighting

    For title matches only include title in final result text. For paragraph matches include 
    10 words on either side of match for context.
    ...
    for result in result_data:
        ...
        for split in splits:
            if split.lower() == result.term:
                amended.append("<a href='/blog/{}/'>".format(result.post_slug) + split + "</a>")
            else:
